...
def track2events(track):
    ...
    group_ids = [i for i in range(len(events)) if events[i][0] in [' '] or events[i][0].isupper()]
...
def get_data_from_musicxml(filepath, bpm, phoneme_durations=None,
    ...
    for k, v in score.items():
        ...
        #print(f"Alignment: {alignment}, {len(alignment)}")
        #print(f"Alignment: {alignment[0]}, {len(alignment[0])}")
        #print(f"Alignment: {alignment[-1], {len(alignment[-1])}}")
        #print(f"f0s: {f0s}")
